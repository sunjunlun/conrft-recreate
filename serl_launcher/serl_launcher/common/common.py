...
class JaxRLTrainState(struct.PyTreeNode):
    ...
    def apply_gradients(self, *, grads: Any) -> "JaxRLTrainState":
        ...
        updates_and_new_states = self._tx_tree_map(
            lambda tx, opt_state, grad: tx.update(
                grad, opt_state, self.params),
            self.txs,
            self.opt_states,
            grads,
        )
        updates = self._tx_tree_map(
            lambda _, x: x[0], self.txs, updates_and_new_states)
    ...
    def apply_loss_fns(
        ...
        grads_and_aux = jax.tree_map(
            lambda loss_fn, rng: jax.grad(
                loss_fn, has_aux=has_aux)(self.params, rng),
            loss_fns,
            rngs,
        )
    ...
    @classmethod
    def create(
        cls, *, apply_fn, params, txs, target_params=None, rng=jax.random.PRNGKey(0), epsilon=0.0, bc_weight=0.5, q_weight=1.0
    ):
        """
